...
func (server *Server) CreateAndSendInvoice(w http.ResponseWriter, r *http.Request) {
	// receive json
	var order Order

	err := server.readJSON(w, r, &order)
	if err != nil {
...
func (server *Server) createInvoicePDF(order Order) error {
